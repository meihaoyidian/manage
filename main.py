...
def format_status(status):
    ...
    status_list = {'0': '新增', '1': '未使用', '2': '使用中', '3': '已归还'}
...
class EquipmentAdmin(sqla.ModelView):
    ...
    form_args = dict(
        ...
        code=dict(label='设备数量', validators=[validators.required()]),
        status=dict(label='设备状态', validators=[validators.required()])
    )


# 创建管理系统
admin = Admin(app, name='设备管理', template_mode='bootstrap3')
...
def build_db():
    ...
    # 通过 excel 导入测试数据
    data = xlrd.open_workbook('demo1.xlsx')
...
if __name__ == '__main__':
    ...
    app.run(host='172.16.150.134', port=5000, debug=True)
